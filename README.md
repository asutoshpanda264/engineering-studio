...
- **Load Balancer** — round-robin or least-connections distribution (selectable, with a per-target request-distribution chart in the Inspector so the two are directly comparable), response pass-through
...
- **Message Queue** — acknowledges the producer the instant a message is durably admitted (not once a consumer finishes it), then dispatches to its own downstream consumer as an independently-simulated request under the same requestId; a bounded backlog absorbs bursts a consumer alone couldn't keep up with, only rejecting once that backlog itself is full (`MetricsCollector`'s `clientIds` filter keeps the queue's own internal dispatch from double-counting against client-facing totals)
